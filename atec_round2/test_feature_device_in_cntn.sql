...
drop table if exists t_sj_test_device_cnt_20n;


create table t_sj_test_device_cnt_20n as
select t3.event_id,
       t4.device_cnt_20n
     ...
     from
         (select row_number() over(partition by t1.device_sign,t1.gmt_occur_unix
          ...
          left outer join
              (select device_sign,
                      gmt_occur_unix
               from t_sj_test_data_code_unix) t2 on t1.device_sign = t2.device_sign
...
-- 统计用户前20次中的变量分布情况
drop table if exists t_sj_test_device_client_ip_20n;


create table t_sj_test_device_client_ip_20n as
select t3.event_id,
       t4.client_ip,
...
drop table if exists t_sj_test_device_network_20n;


create table t_sj_test_device_network_20n as
select t3.event_id,
       t4.network,
...
drop table if exists t_sj_test_device_user_id_20n;


create table t_sj_test_device_user_id_20n as
select t3.event_id,
       t4.user_id,
...
inner join
    (select device_sign,
     ...
     group by device_sign,
              gmt_occur_unix,
              user_id)t4 on t3.device_sign = t4.device_sign
and t3.gmt_occur_unix = t4.gmt_occur_unix;
     drop table if exists t_sj_test_device_info_1_20n;


create table t_sj_test_device_info_1_20n as
select t3.event_id,
       t4.info_1,
...
and t3.gmt_occur_unix = t4.gmt_occur_unix;
     drop table if exists t_sj_test_device_info_2_20n;


create table t_sj_test_device_info_2_20n as
select t3.event_id,
       t4.info_2,
...
and t3.gmt_occur_unix = t4.gmt_occur_unix;
     drop table if exists t_sj_test_device_ip_prov_20n;


create table t_sj_test_device_ip_prov_20n as
select t3.event_id,
       t4.ip_prov,
...
and t3.gmt_occur_unix = t4.gmt_occur_unix;
     drop table if exists t_sj_test_device_ip_city_20n;


create table t_sj_test_device_ip_city_20n as
select t3.event_id,
       t4.ip_city,
...
and t3.gmt_occur_unix = t4.gmt_occur_unix;
     drop table if exists t_sj_test_device_mobile_oper_platform_20n;


create table t_sj_test_device_mobile_oper_platform_20n as
select t3.event_id,
...
and t3.gmt_occur_unix = t4.gmt_occur_unix;
     drop table if exists t_sj_test_device_operation_channel_20n;


create table t_sj_test_device_operation_channel_20n as
select t3.event_id,
...
and t3.gmt_occur_unix = t4.gmt_occur_unix;
     drop table if exists t_sj_test_device_pay_scene_20n;


create table t_sj_test_device_pay_scene_20n as
select t3.event_id,
       t4.pay_scene,
...
and t3.gmt_occur_unix = t4.gmt_occur_unix;
     drop table if exists t_sj_test_device_amt_20n;


create table t_sj_test_device_amt_20n as
select t3.event_id,
       t4.amt,
...
and t3.gmt_occur_unix = t4.gmt_occur_unix;
     drop table if exists t_sj_test_device_opposing_id_20n;


create table t_sj_test_device_opposing_id_20n as
select t3.event_id,
       t4.opposing_id,
...
drop table if exists t_sj_test_device_scatter_freq_20n;


create table t_sj_test_device_scatter_freq_20n as
select t0.event_id,
       case
...
left outer join t_sj_test_device_cnt_20n t1 on t0.event_id = t1.event_id
left outer join t_sj_test_device_client_ip_20n t2 on t0.event_id = t2.event_id
and t0.client_ip = t2.client_ip
left outer join t_sj_test_device_network_20n t3 on t0.event_id = t3.event_id
and t0.network = t3.network
left outer join t_sj_test_device_user_id_20n t4 on t0.event_id = t4.event_id
and t0.user_id = t4.user_id
left outer join t_sj_test_device_info_1_20n t5 on t0.event_id = t5.event_id
and t0.info_1 = t5.info_1
left outer join t_sj_test_device_info_2_20n t6 on t0.event_id = t6.event_id
and t0.info_2 = t6.info_2
left outer join t_sj_test_device_ip_prov_20n t7 on t0.event_id = t7.event_id
and t0.ip_prov = t7.ip_prov
left outer join t_sj_test_device_ip_city_20n t8 on t0.event_id = t8.event_id
and t0.ip_city = t8.ip_city
left outer join t_sj_test_device_mobile_oper_platform_20n t9 on t0.event_id = t9.event_id
and t0.mobile_oper_platform = t9.mobile_oper_platform
left outer join t_sj_test_device_operation_channel_20n t10 on t0.event_id = t10.event_id
and t0.operation_channel = t10.operation_channel
left outer join t_sj_test_device_pay_scene_20n t11 on t0.event_id = t11.event_id
and t0.pay_scene = t11.pay_scene
left outer join t_sj_test_device_amt_20n t12 on t0.event_id = t12.event_id
and t0.amt = t12.amt
left outer join t_sj_test_device_opposing_id_20n t13 on t0.event_id = t13.event_id
